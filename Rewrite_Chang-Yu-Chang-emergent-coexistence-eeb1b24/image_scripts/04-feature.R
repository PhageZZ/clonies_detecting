...
smooth_transect <- function (transect, span = 0.8) {
  ...
  loess_custom <- function(formula, data) {
    n <- nrow(data)
    # 如果数据点太少，动态调整 span
    if (n < 10) {
      span <- 1  # 少于10个数据点时，span取1
    } else {
      span <- min(max(0.1, span), 1)  # 确保 span 在 0.1 到 1 之间
    }
    tryCatch({
      loess(formula, data, span = span)
    }, error = function(e) {
      warning("Loess failed: ", e$message, ". Falling back to linear model.")
      lm(formula, data)  # 如果 loess 失败，使用线性模型回退
    })
  }
  ...
  transect %>%
    # Smooth intensity
    nest(data = -ObjectID) %>%
    mutate(mod = map(data, ~ loess_custom(Intensity ~ DistanceToCenter, .x)),
